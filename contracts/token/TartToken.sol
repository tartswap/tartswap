...
// TART — the TartSwap ecosystem token.
//
// Fixed-supply BEP-20 with an on-trade fee: 10,000,000,000 TART (9 decimals)
// minted once in the constructor, no mint function. Buys and sells pay
// 2% auto-liquidity (LP minted to the dead address) + 3% treasury; plain
// wallet-to-wallet transfers are untaxed. Every fee setter is capped at
// 10% by a require() in this contract.
...
contract TartToken is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("TartSwap", "TART") {
        initializeOwner();
        _mint(owner(), 10_000_000_000 * (10 ** 9));

        liquidityFeeBuy = 200;
        liquidityFeeSell = 200;
        ...
        marketingWallet = 0x12F02a949c730d0A8AB6000a2384bffe82aACb2f;
    ...
    function initializeOwner() internal {
        // CREATE2 vanity deploy makes the factory msg.sender, so ownership is
        // reassigned here. The owner/deployer is the same wallet on every chain
        // now (0x6aB8…e352), so testnet mirrors mainnet exactly.
        if (
            block.chainid == 56 || block.chainid == 1 ||
            block.chainid == 5 || block.chainid == 97
        ) {
            _transferOwnership(0x6aB8Aca85a7f73876Cdc288686be24f67e2ee352);
        }
    }
