...
contract TartSwapRouterV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    /**
     * @notice Removes ETH liquidity for taxed (fee-on-transfer) tokens via PancakeSwap's supporting variant.
     * @dev The LP is pulled to this wrapper and forwarded to Pancake, which sends the underlying token and
     *      BNB straight to `to`. Returns only the BNB amount, mirroring the Pancake supporting-variant signature.
     */
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        address lpToken,
        ...
        address to,
        uint deadline
    ) external nonReentrant whenNotPaused returns (uint amountETH) {
        require(token != address(0) && lpToken != address(0), "Zero token");
        require(to != address(0), "Zero receiver");
        uint256 receivedLiquidity = _pullToken(IERC20(lpToken), msg.sender, liquidity);
        _approveRouter(lpToken, receivedLiquidity);
        amountETH = pancakeRouter.removeLiquidityETHSupportingFeeOnTransferTokens(token, receivedLiquidity, amountTokenMin, amountETHMin, to, deadline);
        emit LiquidityRemoved(msg.sender, token, WBNB, receivedLiquidity, to);
    }
